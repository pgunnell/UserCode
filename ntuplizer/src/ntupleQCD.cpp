...
void ntupleQCD::analyze(size_t childid /* this info can be used for printouts */){
	...
	//jets
        ...
        std::vector<float> pt_;         std::vector<float> btag_ ;  std::vector<float> btagAlgo_ ;         std::vector<float> eta_;           std::vector<float> phi_;         std::vector<float> mass_;      std::vector<float> jetflavour_ ;
	...
	for(size_t eventno=0;eventno<nevents;eventno++){
	  ...
	  for(size_t i=0;i<genpart.size();i++){
	    ...
	    if(abs(genpart.at(i)->PID)>=9000000 && abs(genpart.at(i)->PID)<=9000009 && genpart.at(i)->Status==22){
